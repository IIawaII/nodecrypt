...
wss.on("connection", (connection) => {
  ...
  for (const clientId in clients) {
    if (clients[clientId].seen < seenThreshold) {
      try {
  ...
  connection.on("message", (message) => {
    if (!isString(message) || !clients[clientId]) {
      return;
    }
    ...
    if (!clients[clientId].shared && message.length < 2048) {
      try {
        ...
        clients[clientId].shared = keys
          .computeSecret(message, "hex", null)
          .slice(8, 40);

        sendMessage(
          connection,
          publicKey.toString("hex") + "|" + signature.toString("base64")
        );
      } catch (error) {
    ...
    if (clients[clientId].shared && message.length <= 8 * 1024 * 1024) {
...
const handleJoinChannel = (clientId, decrypted) => {
  if (!isString(decrypted.p) || clients[clientId].channel) {
    return;
  }
...
const handleChannelMessage = (clientId, decrypted) => {
  if (!isObject(decrypted.p) || !clients[clientId].channel) {
    return;
  }
...
const sendMessage = (connection, message) => {
  try {
    if (connection.readyState && connection.readyState === ws.OPEN) {
...
const encryptMessage = (message, key) => {
  ...
  try {
    ...
    const paddedBuffer =
      messageBuffer.length % 16 !== 0
        ? Buffer.concat([
            messageBuffer,
            Buffer.alloc(16 - (messageBuffer.length % 16)),
          ])
        : messageBuffer;
    ...
    encrypted =
      iv.toString("base64") +
      "|" +
      cipher.update(paddedBuffer, "", "base64") +
      cipher.final("base64");
  } catch (error) {
...
const decryptMessage = (message, key) => {
  ...
  try {
    ...
    const decryptedText =
      decipher.update(parts[1], "base64", "utf8") + decipher.final("utf8");
