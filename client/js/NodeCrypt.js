...
class NodeCrypt {
  ...
  async onOpen() {
    ...
    try {
      ...
      this.sendMessage(
        Buffer.from(
          await crypto.subtle.exportKey("raw", this.serverKeys.publicKey)
        ).toString("hex")
      );
    } catch (error) {
  ...
  async onMessage(event) {
        ...
        if (
          ...
          if (this.callbacks.onServerSecured) {
            try {
              ...
              this.logEvent(
                "onMessage-server-secured-callback",
                error,
                "error"
              );
            }
    ...
    const serverDecrypted = this.decryptServerMessage(
      event.data,
      this.serverShared
    );
    ...
    if (
      ...
      try {
        ...
        this.channel[serverDecrypted.c].shared = Buffer.from(
          this.xorHex(
            this.channel[serverDecrypted.c].keys
              .derive(
                this.clientEc
                  .keyFromPublic(serverDecrypted.p, "hex")
                  .getPublic()
              )
              .toString("hex")
              .padEnd(64, "8")
    ...
    if (
      ...
      const clientDecrypted = this.decryptClientMessage(
        serverDecrypted.p,
        this.channel[serverDecrypted.c].shared
      );
      ...
      if (
        ...
        this.channel[serverDecrypted.c].username = clientDecrypted.p
          .replace(/^\s+/, "")
          .replace(/\s+$/, "");
      ...
      if (
        ...
        (this.isString(clientDecrypted.d) || this.isObject(clientDecrypted.d))
      ) {
  ...
  encryptServerMessage(message, key) {
    ...
    try {
      ...
      encrypted =
        iv.toString("base64") +
        "|" +
        Buffer.from(cipher.encrypt(message)).toString("base64");
    } catch (error) {
  ...
  decryptServerMessage(message, key) {
    ...
    try {
      ...
      decrypted = JSON.parse(
        Buffer.from(decipher.decrypt(Buffer.from(parts[1], "base64")))
          .toString("utf8")
          .replace(/\0+$/, "")
      );
    } catch (error) {
  ...
  decryptClientMessage(message, key) {
    ...
    try {
      ...
      decrypted = JSON.parse(
        Buffer.from(decipher.decrypt(Buffer.from(parts[2], "base64")))
          .toString("utf8")
          .replace(/\0+$/, "")
      );
    } catch (error) {
  ...
  xorHex(a, b) {
    ...
    for (let i = 0; i < hexLength; ++i) {
      result += (
        parseInt(a.charAt(i), 16) ^ parseInt(b.charAt(i), 16)
      ).toString(16);
    }
    return result;
  }
