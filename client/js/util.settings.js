...
// 从 themes 文件夹直接导入 THEMES
import { THEMES } from "../themes/index.js";
// 从 util.theme.js 导入其他函数
import { getCurrentTheme, applyTheme } from "./util.theme.js";
...
// 导入通知工具函数
import { handleNewMessage } from "./util.notification.js";
...
function saveSettings(settings) {
  const { notify, sound, theme, language } = settings;
...
function setupSettingsPanel() {
  ...
  settingsContent.innerHTML = `
		<div class="settings-section">
			<div class="settings-section-title">${t(
        ...
        "Theme Settings"
          ...
          }" data-theme-id="${theme.id}" style="background: ${
            theme.background
  ...
  on(notifyCheckbox, "change", (e) => {
    ...
    if (checked) {
      ...
      // Use native API, logic is now centralized in util.notification but we trigger permission here for UI
      // 使用原生 API，逻辑现在集中在 util.notification，但我们在这里触发权限以进行 UI 交互
...
export function notifyMessage(roomName, msgType, text, sender) {
  // Delegate to centralized notification utility
  // 委托给集中式通知工具
  handleNewMessage(roomName, msgType, text, sender);
}
