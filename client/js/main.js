...
window.config = {
  wsAddress: `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${
    window.location.host
  }`, // WebSocket 服务器地址 / WebSocket server address
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function sendMessage() {
    ...
    if (rd && rd.chat) {
      if (images.length > 0) {
        ...
        images.forEach(imgData => {
          const messageContent = {
             text: text, // 如果有配文
             ...imgData // 包含 originalId, thumbId, key
          };
          
          // 发送逻辑 (复用原本的加密通道发送 JSON)
          if (rd.privateChatTargetId) {
             // ... 私聊逻辑，type 改为 'image_r2_private'
        ...
        if (rd.privateChatTargetId) {
          ...
          if (targetClient && targetClient.shared) {
            ...
            const encryptedClientMessage = rd.chat.encryptClientMessage(
            ...
            const encryptedMessageForServer = rd.chat.encryptServerMessage(
              serverRelayPayload,
            ...
            addSystemMsg(
              `${t(
                "system.private_message_failed",
                "Cannot send private message to"
              )} ${rd.privateChatTargetName}. ${t(
                "system.user_not_connected",
                "User might not be fully connected."
              )}`
            );
          }
        } else {
      ...
      if (
        imagePasteHandler &&
        typeof imagePasteHandler.refreshPlaceholder === "function"
      ) {
