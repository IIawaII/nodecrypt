...
function copyToClipboard(
  text,
  successMessage = t("action.copied", "Copied to clipboard!"),
  errorPrefix = t("action.copy_failed", "Copy failed, url:")
) {
  if (!text) {
    window.addSystemMsg &&
      window.addSystemMsg(t("action.nothing_to_copy", "Nothing to copy"));
    return;
  }
...
function showFallbackCopy(text, prefix) {
  if (typeof prompt === "function") {
    ...
    window.addSystemMsg &&
      window.addSystemMsg(
        t("action.copy_not_supported", "Copy not supported in this environment")
      );
  }
}
...
function executeMenuAction(action, closeMenuCallback) {
  try {
    ...
    window.addSystemMsg &&
      window.addSystemMsg(
        t("action.action_failed", "Action failed. Please try again.")
      );
  } finally {
...
function handleShareAction() {
  ...
  if (!validation.valid) {
    window.addSystemMsg &&
      window.addSystemMsg(
        `${t("action.cannot_share", "Cannot share:")} ${validation.error}`
      );
    return;
  }
  ...
  let url = `${location.origin}${location.pathname}?r=${encodeURIComponent(
    encryptedRoom
  )}`;
  if (encryptedPwd) {
  ...
  copyToClipboard(
    url,
    t("action.share_copied", "Share link copied!"),
    t("action.copy_url_failed", "Copy failed, url:")
  );
}
...
export function setupMobileUIHandlers() {
  ...
  if (mobileMenuBtn && sidebar && sidebarMask) {
    ...
    sidebarMask.onclick = function () {
      ...
      if (
        settingsSidebar &&
        settingsSidebar.classList.contains("mobile-open")
      ) {
        closeSettingsPanel();
      } else {
  ...
  document.addEventListener("click", function (ev) {
    ...
    const isSettingsButtonClick =
      settingsBtn && settingsBtn.contains(ev.target);
    const isSettingsBackButtonClick =
      $id("settings-back-btn") && $id("settings-back-btn").contains(ev.target);
    ...
    if (
      ...
      if (
        !settingsSidebar.contains(ev.target) &&
        !isSettingsButtonClick &&
        !isSettingsBackButtonClick
      ) {
        closeSettingsPanel();
      }
    }
...
export function createUserItem(user, isMe) {
  ...
  div.className =
    "member" +
    (isMe ? " me" : "") +
    (isPrivateTarget ? " private-chat-active" : "");
  const rawName = user.userName || user.username || user.name || "";
  const safeUserName = escapeHTML(rawName);
  div.innerHTML = `<span class="avatar"></span><div class="member-info"><div class="member-name">${safeUserName}${
    isMe ? t("ui.me", " (me)") : ""
  }</div></div>`;
...
export function loginFormHandler(modal) {
  return function (e) {
    ...
    const exists = roomsData.some(
      (rd) =>
        rd.roomName && rd.roomName.toLowerCase() === roomName.toLowerCase()
    );
    if (roomInput) {
...
export function generateLoginForm(isModal = false) {
  ...
  return `		<div class="input-group">
			...
			<label for="userName${idPrefix}" class="floating-label">${t(
    "ui.username",
    "Username"
  )}</label>
		</div>
		<div class="input-group">
			...
			<label for="roomName${idPrefix}" class="floating-label">${t(
    "ui.node_name",
    "Node Name"
  )}</label>
		</div>
		<div class="input-group">
			<input id="password${idPrefix}" type="password" autocomplete="${
    isModal ? "off" : "current-password"
  }" minlength="1" maxlength="15" placeholder="">
			<label for="password${idPrefix}" class="floating-label">${t(
    "ui.node_password",
    "Node Password"
  )} <span class="optional">${t("ui.optional", "(optional)")}</span></label>
		</div>
...
export function openLoginModal() {
  ...
  modal.innerHTML = `<div class="login-modal-bg"></div><div class="login-modal-card"><button class="login-modal-close login-modal-close-abs">&times;</button><h1>${t(
    "ui.enter_node",
    "Enter a Node"
  )}</h1><form id="login-form-modal">${generateLoginForm(true)}</form></div>`;
