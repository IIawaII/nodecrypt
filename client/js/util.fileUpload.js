...
function updateModalTexts() {
  ...
  if (dragDropText) {
    dragDropText.innerHTML = `
			...
			<p>${t("file.or", "or")} <button class="file-browse-btn" type="button">${t(
      "file.browse_files",
      "browse files"
    )}</button></p>
		`;
      ...
      "browse files"
				...
				<div class="file-list-container" id="file-list-container" style="display: none;">
					<div class="file-list-header">
						<span class="file-list-title">${t(
              "file.selected_files",
              "Selected Files"
            )}</span>
						<button class="file-clear-all-btn" type="button">${t(
              "file.clear_all",
              "Clear All"
            )}</button>
					</div>
			...
			<div class="file-upload-footer">
				<button class="file-upload-cancel-btn" type="button">${t(
          "file.cancel",
          "Cancel"
        )}</button>
				<button class="file-upload-send-btn" type="button" disabled>${t(
          "file.send_files",
          "Send Files"
        )}</button>
			</div>
...
function updateFileList() {
  ...
  const totalSize = Array.from(selectedFiles.values()).reduce(
    (sum, file) => sum + file.size,
    0
  );
  const summaryText = t(
    "file.files_selected",
    "{count} files selected, {size} total"
  )
...
async function handleSendFiles() {
  ...
  try {
    ...
    if (window.addSystemMsg) {
      window.addSystemMsg(
        `${t("system.file_send_failed", "Failed to send files:")} ${
          error.message
        }`
      );
    }
