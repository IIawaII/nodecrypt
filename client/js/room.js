...
export function renderRooms(activeId = 0) {
  ...
  roomsData.forEach((rd, i) => {
    ...
    if (rd.unreadCount && i !== activeId) {
      unreadHtml = `<span class="room-unread-badge">${
        rd.unreadCount > 99 ? "99+" : rd.unreadCount
      }</span>`;
    }
...
export function handleClientSecured(idx, user) {
  ...
  const existingUserIndex = rd.userList.findIndex(
    (u) => u.clientId === user.clientId
  );
...
export function handleClientMessage(idx, msg) {
  ...
  if (
    msg.clientId === newRd.myId &&
    msg.userName === newRd.myUserName &&
    !msg.type.includes("_private")
  ) {
    return;
  }
  ...
  if (msgType.startsWith("file_")) {
    ...
    if (msgType === "file_start" || msgType === "file_start_private") {
      ...
      if (!realUserName && msg.clientId && newRd.userMap[msg.clientId]) {
        realUserName =
          newRd.userMap[msg.clientId].userName ||
          newRd.userMap[msg.clientId].username ||
          newRd.userMap[msg.clientId].name;
      }
      const historyMsgType =
        msgType === "file_start_private" ? "file_private" : "file";

      const fileId = msg.data && msg.data.fileId;
      if (fileId) {
        // Only proceed if we have a fileId
        const messageAlreadyInHistory = newRd.messages.some(
          (m) =>
            m.msgType === historyMsgType &&
            m.text &&
            m.text.fileId === fileId &&
            m.userName === realUserName
        );
      ...
      const notificationMsgType = msgType.includes("_private")
        ? "private file"
        : "file";
