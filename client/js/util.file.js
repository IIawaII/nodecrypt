      ...
      inflate(compressed, async (err, decompressed) => {
        ...
        if (originalHash) {
          try {
            ...
            reject(
              new Error("File integrity check failed: " + hashError.message)
            );
            return;
          }
        }
...
// 将归档分卷解压为多个文件
async function decompressArchiveToFiles(
  volumes,
  fileManifest,
  archiveHash = null
) {
    ...
    return new Promise((resolve, reject) => {
      inflate(compressed, async (err, decompressed) => {
        ...
        if (archiveHash) {
          try {
            ...
            if (calculatedHash !== archiveHash) {
              reject(
                new Error("Archive integrity check failed: hash mismatch")
              );
              return;
            }
          } catch (hashError) {
            reject(
              new Error("Archive integrity check failed: " + hashError.message)
            );
            return;
          }
        }
        ...
        for (const fileInfo of fileManifest) {
          ...
          const nameLengthBytes = decompressed.slice(
            dataOffset,
            dataOffset + 4
          );
          const nameLengthView = new DataView(nameLengthBytes.buffer);
          ...
          const nameBytes = decompressed.slice(
            dataOffset,
          ...
          const fileData = decompressed.slice(
            dataOffset,
            dataOffset + fileSize
          );
          dataOffset += fileSize;
      ...
      showFileUploadModal(async (files) => {
        ...
        for (const imgFile of imageFiles) {
          try {
            ...
            const metaData = await processAndUploadImage(imgFile);
            // 补充 userName，并通过回调发送
            onSend({ ...metaData, userName });
          } catch (error) {
            console.error("Image upload failed via attachment:", error);
            if (window.addSystemMsg) {
              window.addSystemMsg(`Image upload failed: ${imgFile.name}`);
            }
          }
        }
...
async function handleFilesUpload(files, onSend) {
  ...
  try {
    ...
    if (files.length === 1) {
      ...
      const { volumes, originalSize, compressedSize, originalHash } =
        await compressFileToVolumes(file);
      ...
      const {
        ...
        fileCount,
        fileManifest,
      } = await compressFilesToArchive(files);
...
function updateFileProgress(fileId) {
  ...
  elements.forEach((element) => {
    ...
    if (transfer.status === "sending") {
      ...
      if (statusText)
        statusText.textContent = `Sending ${transfer.sentVolumes}/${transfer.totalVolumes}`;
      if (downloadBtn) {
      ...
      if (progressBar) progressBar.style.width = `${progress}%`;
      if (statusText)
        statusText.textContent = `Receiving ${transfer.receivedVolumes.size}/${transfer.totalVolumes}`;
      if (downloadBtn) {
        downloadBtn.classList.remove("show", "animate-in");
        downloadBtn.style.display = "none";
      }
...
export async function downloadFile(fileId) {
  ...
  try {
    if (transfer.isArchive) {
      ...
      await decompressArchiveToFiles(
        transfer.volumeData,
        transfer.fileManifest,
        transfer.archiveHash
      );
      // 删除系统提示
    } else {
      // Download single file
      await decompressVolumesToFile(
        transfer.volumeData,
        transfer.fileName,
        transfer.originalHash
      );
      // 删除系统提示
    }
  } catch (error) {
